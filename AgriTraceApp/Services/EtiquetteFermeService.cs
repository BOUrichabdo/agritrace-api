...
namespace AgriTraceApp.Services
{
    public class EtiquetteFermeService
    {
        ...
        // GENERER ETIQUETTE avec societeId
        public async Task<EtiquetteDto?> GenererEtiquette(CreateEtiquetteDto dto, int societeId)
        {
            try
            {
                var response = await _httpClient.PostAsJsonAsync($"EtiquetteFerme/generer?societeId={societeId}", dto);
                if (!response.IsSuccessStatusCode) return null;
                return await response.Content.ReadFromJsonAsync<EtiquetteDto>();
            }
            catch (Exception ex)
            {
        ...
        // GET ETIQUETTE BY CODE avec societeId
        public async Task<EtiquetteDto?> GetEtiquetteByCode(string code, int societeId)
        {
            try
            {
                return await _httpClient.GetFromJsonAsync<EtiquetteDto>($"EtiquetteFerme/bycode/{code}?societeId={societeId}");
            }
            catch (Exception ex)
            {
                Console.WriteLine(ex.Message);
        ...
        // GET PDF (pas de filtre societe, car l'URL est externe, mais on peut ajouter si nécessaire)
