...
public partial class Ferme : ContentPage
{
    ...
    private async Task LoadAgriculteurs()
    {
        try
        {
            _agriculteurs = await _serviceafriculteur.GetAgriculteurs(societeId);
            CMBOAGRI.ItemsSource = _agriculteurs;
            Debug.WriteLine($"Agriculteurs chargés : {_agriculteurs?.Count ?? 0}");
        }
        catch (Exception ex)
        {
            await DisplayAlert("Erreur", $"Impossible de charger les agriculteurs : {ex.Message}", "OK");
        }
    }
    ...
    private async Task LoadData()
    {
        try
        {
            // Récupération des fermes par societeId
            _fermes = await _servicesferme.GetFermes(societeId);
            ListeFerme.ItemsSource = _fermes;

            // Chargement des agriculteurs pour le combobox
            await LoadAgriculteurs();
        }
        catch (HttpRequestException ex)
        {
            // Afficher le message d'erreur détaillé (ex: "Erreur 400 : ...")
            await DisplayAlert("Erreur", $"Impossible de charger les fermes : {ex.Message}", "OK");
        }
        catch (Exception ex)
        {
    ...
    private async void VALIDER_Clicked(object sender, EventArgs e)
    {
        ...
        if (isEditMode)
        {
            ...
            await _servicesferme.UpdateFerme(agri, societeId);
        ...
        else
        {
            try
            {
                ...
                await _servicesferme.AddFerme(Ferme, societeId);
    ...
    private async void DELETE_Invoked(object sender, EventArgs e)
    {
        ...
        await _servicesferme.DeleteFerme(agri.Id, societeId);
        await LoadData();
